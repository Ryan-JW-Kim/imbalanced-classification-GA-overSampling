...
def execute_persist_retrain(data_key, scheme_key):
...
def execute_noPersist_retrain(data_key, scheme_key):
	...
	for generation_iter in range(5):	
		...
		new_x_train, new_y_train = combine_sets(x_train, y_train, synthetic_survivors, [minority_labels[0]] * len(synthetic_survivors))
...
def execute_persist_noTrain(data_key, scheme_key):
	...
	logger = wandb.init(
		project="GA Instance Selection Over Sampling", 
		group=f"{scheme_key}:{dataset_name}:persist-noTrain",
		tags=['2025-06-06', dataset_name],		
		name=data_key
	)
...
def execute_noPersist_noTrain(data_key, scheme_key):
	...
	for generation_iter in range(5):	
		...
		iteration_history.append(result, candidate_x_train, candidate_y_train)
...
if __name__ == "__main__":
	...
	for dataset in ['yeast4', 'ecoli4', 'winequality-red-8_vs_6-7']:
		for data_key in splits[dataset]:
			try:
				synthetic_samples = execute_persist_retrain(data_key, "persist_retrain")
				synthetic_samples = execute_persist_noTrain(data_key, "persist_noTrain")
				synthetic_samples = execute_noPersist_retrain(data_key, "noPersist_retrain")
				synthetic_samples = execute_noPersist_noTrain(data_key, "noPersist_noTrain")
